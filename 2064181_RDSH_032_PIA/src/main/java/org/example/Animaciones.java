...
public class Animaciones extends JPanel {
 ...
 private BufferedImage background, cursor, welcomeBackground,
 loseBackground, winBackground;
 ...
 System.err.println("Error cargando imágenes de dulces o rayadas");
 dulcesImages = null;
 ...
 System.err.println("Error cargando imagen de chocolate");
 ...
 public void updateAnimacion(Dulce[][] grid, int x0, int y0, int click, int score, int moves,
                             boolean showWelcome, boolean showLose, boolean showWin) {
 this.tablero = grid;
 this.currentX0 = x0;
 this.currentY0 = y0;
 ...
 if (background != null) {
 g2.drawImage(background, 0, 0, WIDTH, HEIGHT, null);} else {
 ...
 for (int j = 1; j <= 8; j++) {
 if (tablero[i][j] != null) {
 BufferedImage dulceImage = getDulceImage(tablero[i][j].tipo, tablero[i][j].isChocolate,
         tablero[i][j].isStripedVertical, tablero[i][j].isStripedHorizontal);
 if (dulceImage != null) {
 ...
 if (isChocolate) {return chocolateImage;}
