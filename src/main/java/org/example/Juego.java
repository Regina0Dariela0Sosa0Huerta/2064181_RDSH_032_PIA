...
public class Juego {
    private Dulce[][] tablero;//matriz 2x2 para la cuadricula
    ...
    private final int puntaje_ganador = 150; // Puntaje ganador
    ...
    private void efecto_bomba() {
        ...
        for (int i = 1; i <= 8; i++) {//recorre toda la matriz
            ...
            }//match 2: match de 4 o mas dulces y se van a eliminar todas
        }
    ...
    public void updateGame() {
        //Match 2 eliminara todos los dulces y Match 3 eliminara toda la fila o columna
        //entonces si tienen match 2 o 3 tarde o temprano se van a eliminar entonces no se cambia a match 0
        // y las otras gemas que no estan en proceso de  eliminarse tienen que "reiniciarse"
        ...
        // si son dulces normales durante el transcurso del juego pues tiene que eliminar esos dulces
        if (!bomba_activada && !rayado_activado) {
            detecta_Matches();
        }
        ...
        int sumar_este_puntaje = 0;//0 pq no se suma al puntaje todo el puntaje que ya se tenia mas el nuevo puntaje obtenido por la nueva combinacion
        for (int i = 1; i <= 8; i++) {
            for (int j = 1; j <= 8; j++) {
                if (tablero[i][j].match > 0) sumar_este_puntaje++;
            }
        }
    ...
    private void detecta_Matches() {
        ...
        for (int i = 1; i <= 8; i++) {//recorrera la fila
            ...
            for (int j = 2; j <= 8; j++) {//recorrera la columna siguiente hasta el final
                if (tablero[i][j].tipo == currentKind && !tablero[i][j].isChocolate && !tablero[i][j].isStripedHorizontal && !tablero[i][j].isStripedVertical) {
    ...
    //si anteriormente se marco un match
